"""Run a queue of P3 Dream TextMAS shards across GPUs.

This local-only launcher executes ``p3_run_dream_textmas_gate.py`` shards with a
frozen Dream generation config. It is intended for long locked evaluations such
as held-out 800. It does not train or create SwanLab runs.
"""
...
RUNNER = REPO_ROOT / "drla/scripts/p3_run_dream_textmas_gate.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--shard-size", type=int, default=70)
...
def run_queue(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        ...
        "execution_boundary": [
            "local-only P3 Dream shard queue",
            "launches inference/eval shards only",
...
def shard_complete(output_dir: Path) -> bool:
    ...
    if not summary_path.exists() or not generations_path.exists():
        return False
    try:
    ...
    actual_rows = sum(1 for line in generations_path.read_text(encoding="utf-8").splitlines() if line.strip())
