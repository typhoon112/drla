...
5. **P1 必须做结构消融与校准复盘**。已完成的负结果包括 `all_tokens`、`pma1`、`mean_max`、`d32_pma4`、`d128_pma4`、`stabilityw2`、`no_block_budget`、简单 `film` gating、只改 `per_task` 校准、SQuAD 单任务否决的 `last_process_query` readout context、只加 `completion_risk` 辅助 head 的 SQuAD 诊断，以及 MMLU-focused `contentfulw1`、`empty_answer_risk`、`answer_format_risk` 训练诊断。当前公平口径：`answer_identity_action + completion_risk` 为 `47` losses / `617` mismatches / `1.742/4` blocks；`answer_identity_halt + completion_risk` 为 `31` / `699` / `1.824/4`。learned action->halt gate v2 的 fair source-valid cost 为 `10` / `315` / `2.220/4`，backfilled source-cost-limited 为 `10` / `465` / `1.859/4`，test oracle tight-cost 诊断为 `25` / `461` / `1.782/4`。LAMBADA `utility_mse`、raw utility-score threshold、`utility_pairwise`、`utility_pairwise --utility-mismatch-penalty 1.0`、`utility_soft_bce` temperature `1.0/0.1`、2026-05-27 四任务 `decomposed_expected_utility` partial、post-hoc scalar utility-weight calibration，以及 constrained head-threshold selector 都没有明确超过 v2。source-task-robust calibration 在四任务 decomposed subset 上给出第一个 deployable cost-limited 改善（`0` losses / `93` mismatches / `1.876/4`），但还不是 official8 结论。`train_boundary_stratified` 在 LAMBADA/SQuAD/HellaSwag 三任务局部复盘中只对 SQuAD tight mismatch 有收益，HellaSwag 变差，不能直接扩成 8-task 主线。结论：latent heads 有边界信号，但 plain BCE / cost-weighted BCE / 朴素 utility regression / 简单 pairwise / soft utility BCE / 仅重采样 rare-boundary valid / 简单多头 expected-utility / 简单标量校准 / 两阶段阈值约束仍只是在 safety-cost-mismatch 间移动；下一步补完 remaining four held-out tasks，并用 task-robust calibration 决定是否继续 risk-control 方向。
   原因：架构选择不能只靠直觉定。
...
- `/data1/luyifei/drla/docs/P1_Progress_and_Literature_Synthesis_2026-05-27.md`：2026-05-27 P1 进展、文献复盘和 decomposed expected-utility gate partial 负结果。
