...
48. 2026-05-27 已开始 P1 locked evaluation。首组为 seed66 最优 `trajectory_token + answer_identity_action + completion_risk + answer_identity_stability` 模型，fresh split seed `20260601`，local-only eval，路径 `/data1/luyifei/drla/outputs/cola_latent_halt_student_eval_locked/cross_task_full_b64_bs12_seed66_d64_pma4_trajtok_answer_identity_action_completionrisk_identitystable_split20260601_riskcert_20260527/subseed20260601`，聚合 summary `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_locked_seed66_split20260601_riskcert_20260527/summary.json`。8-task micro summary：`4980` samples，selected accuracy `20.763%`，fixed-final accuracy `20.803%`，avg blocks `1.863/4`，`2` losses vs final，`23` prediction mismatches vs final，`25` prediction mismatches vs prediction-stability。`halt_decisions_*.jsonl` 已包含 `readiness_state.version=p1_latent_halt_student_v1` 与 `online_inputs=latent_prefix_and_process_features_only`。注意：这是 locked eval 的第一组 seed，不应根据该结果回调模型或阈值；若作为 paper 主表，需要按同一冻结协议补齐 seed67/seed68 或明确声明只报告 seed66 locked audit。
