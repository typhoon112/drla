...
> 状态：历史实施日志 / P0-P1 roadmap。本文保留 official Cola + readiness/halt 路线的长实验记录；当前 P2 实施方案以 `/data1/luyifei/drla/docs/P2_Latent_MAS_Communication_Implementation_Plan_2026-05-29.md` 为准。
...
46c. 已完成 `trajectory_token + answer_identity_action + completion_risk` full official8 3-seed LOTO 架构实验。代码在 `train_cola_latent_halt_student.py` 增加 `--process-interaction-mode trajectory_token`：保留原 process token，同时把每个 block 的 pooled state、相邻 block delta 和 process state 合成 trajectory token，进入 causal inter-block Transformer。训练根目录为 `/data1/luyifei/drla/outputs/cola_latent_halt_student_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_20260527`，共 `24` 个训练，全部使用 SwanLab cloud + CUDA，写 `metrics.jsonl`、`best_checkpoint.pt`、`last_checkpoint.pt`。Target-calibration cap128 boundary-penalty eval 根目录为 `/data1/luyifei/drla/outputs/cola_latent_halt_student_eval_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_targetcal_cap128_boundarypen02_subseeds_20260527`，聚合脚本为 `drla/scripts/aggregate_cola_latent_halt_student_subseed_loto.py`，summary 在 `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_boundarypen02_cross_seed_20260527/summary.json`。结果：`41` losses / `806` mismatches / `1.711/4` blocks；旧 action 同口径为 `47` / `617` / `1.742/4`，strict halt 为 `31` / `699` / `1.824/4`。trajectory token 降低 LAMBADA 和 SQuAD loss（LAMBADA `32 -> 28`，SQuAD `15 -> 2`），但新增 MMLU/HellaSwag loss（`0 -> 5`，`0 -> 6`）并增加 mismatch。Wilson audit summary `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_wilson_risk_control_20260527/summary.json` 仍无法在 `<=0.002` loss target 下覆盖 `120/120` folds。结论：显式 latent trajectory/delta readout 是正向信号，但不是主线终点；下一步应把 answer identity / stability 做成轨迹级训练目标或改正式 calibration 数据协议，不能只把 trajectory token 当作“更强模型”直接替代旧 route。
46d. 已完成 `trajectory_token + answer_identity_action + completion_risk + answer_identity_stability` full official8 3-seed LOTO。`answer_identity_stability` 是 decoder/text teacher target：当当前 block 的 `scored_prediction` 已经等于 rollout prediction-stability/final reference 时为正；在线输入仍只使用 raw latent prefix blocks 与 latent/process features。训练根目录为 `/data1/luyifei/drla/outputs/cola_latent_halt_student_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_identitystable_20260527`，共 `24` 个训练，全部 CUDA/GPU + SwanLab cloud，`valid_interval=50`，写 `metrics.jsonl`、`best_checkpoint.pt`、`last_checkpoint.pt`。Target-calibration cap128 boundary-penalty eval 根目录为 `/data1/luyifei/drla/outputs/cola_latent_halt_student_eval_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_identitystable_targetcal_cap128_boundarypen02_subseeds_20260527`，summary 在 `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_identitystable_boundarypen02_cross_seed_20260527/summary.json`。结果：`4` losses / `606` mismatches / `1.812/4` blocks；相比旧 action `47` / `617` / `1.742`、trajectory-only `41` / `806` / `1.711` 和 v2 cost-limited `10` / `465` / `1.859`，这是当前 P1 student-only 低 loss/cost frontier 的最好点。剩余 4 个 repeated losses 对应 2 个 unique empty-answer boundary misses；硬 `contentful>=0.5` gate 可到 `0` losses / `151` mismatches，但成本升至 `2.924/4`，因此只作为诊断。Wilson risk-control summary `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_identitystable_wilson_risk_control_20260527/summary.json` 仍无法认证严格低风险。结论：answer identity/stability teacher head 是明确正向方向，但下一步应处理 empty-answer boundary、text-identity mismatch 和 calibration 可认证性，而不是宣称 safety 完成。
46e. 已完成 `trajectory_token + answer_identity_action + completion_risk + empty_answer_risk + answer_identity_stability` full official8 3-seed LOTO，并将其判为负结果。代码新增 selection metric `readiness_prediction_change_completion_empty_identity_mean_auroc`，避免训练 empty head 却完全不看它选 checkpoint。训练根目录 `/data1/luyifei/drla/outputs/cola_latent_halt_student_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_emptyrisk_identitystable_20260527`，共 `24` 个训练，全部 CUDA/GPU + SwanLab cloud；eval 根目录 `/data1/luyifei/drla/outputs/cola_latent_halt_student_eval_ablation/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_emptyrisk_identitystable_targetcal_cap128_boundarypen02_subseeds_20260527`，共 `120` 个 local-only eval。聚合 summary `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_emptyrisk_identitystable_boundarypen02_cross_seed_20260527/summary.json`：`24` losses / `623` mismatches / `1.829/4` blocks，差于无 empty head 的 `4` / `606` / `1.812`。失败样本不是纯 empty，而是 prefix/continuation identity：`quarant -> quarantined`、`John -> John Vanderbank's workshop`、`27 -> 27 September 2001`、`Metro Trains -> Metro Trains Melbourne` 等。Wilson audit `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_emptyrisk_identitystable_wilson_risk_control_20260527/summary.json` 仍为 `0/120` strict complete folds；empty threshold cap 诊断 `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_trajtok_answer_identity_action_completionrisk_emptyrisk_identitystable_empty_cap_diagnostic_20260527/summary.json` 只有在 `empty_answer_risk_threshold<=0.005` 时完整 zero loss，但成本 `3.808/4`。结论：辅助头存在 negative transfer / proxy mismatch 风险；参考 auxiliary loss gradient similarity、negative transfer 与 early-exit risk-control 文献后，下一步不要继续堆窄二分类头，而应把 prefix/continuation answer identity risk 做成与 halt 决策同构的目标或 calibration 约束。

47. P1 阶段已冻结为 `LatentHaltStudent-v1` verifier v1，下一步不再把“建立 decoder-free verifier”作为 agent-to-agent 的前置任务；P1 本身已经是 decoder-free online-input verifier。进入 agent-to-agent latent communication 前先完成三项工程验收：第一，fresh seed / fresh split locked evaluation，用未被开发过程反复观察过的 split seed 重新划分 target valid/test，并冻结模型结构、head、threshold grid 与 calibration rule，只报告一次 locked test；第二，改进 calibration / risk-control 协议，把当前 `target-valid zero-loss + mismatch cap + boundary penalty` 升级为可记录 Wilson upper bound 的 risk certificate；第三，输出 `readiness_state`，把多头 score、threshold、margin、是否 fallback 到 final block 等信息传给后续 agent，而不是只传一个 halt bit。为此 `eval_cola_latent_halt_student.py` 已支持 `--split-seed-override`、`--calibration-loss-risk-target`、`--calibration-mismatch-risk-target`、`--calibration-risk-bound-z`，并在 `summary.json` 与 `halt_decisions_*.jsonl` 中记录相关字段。后续 agent-to-agent v1 应传递 latent memory + P1 halt action + readiness_state。
48. 2026-05-27 已完成 P1 locked evaluation v1：冻结 `trajectory_token + answer_identity_action + completion_risk + answer_identity_stability` 最优模型，补齐 seed66/67/68 official8 LOTO，fresh split seed `20260601`，local-only eval，没有根据 locked result 回调模型或阈值。三 seed eval roots 为 `/data1/luyifei/drla/outputs/cola_latent_halt_student_eval_locked/cross_task_full_b64_bs12_seed{66,67,68}_d64_pma4_trajtok_answer_identity_action_completionrisk_identitystable_split20260601_riskcert_20260527`，聚合 summary `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_locked_seed66_67_68_split20260601_riskcert_20260527/summary.json`，loss case audit `/data1/luyifei/drla/outputs/cola_experiment_summaries/official8_full_b64_bs12_p1_locked_seed66_67_68_split20260601_riskcert_20260527/loss_case_audit.json`。24-fold / `14940` repeated samples：selected accuracy `20.930%`，fixed-final accuracy `20.950%`，prediction-stability accuracy `20.957%`，avg blocks `1.834/4`，prediction-stability avg blocks `2.501/4`，saving `0.667` blocks；`3` losses vs final、`4` losses vs prediction-stability、`85` mismatches vs final、`91` mismatches vs prediction-stability。Risk certificate 为 `21/24` folds satisfying calibration upper-bound targets；3 个未认证 fold 全部是 OBQA，因为 target-valid 样本太少导致 zero-observed-risk 的 Wilson upper bound 仍为 `0.0838`，超过当前 `0.08` target。`halt_decisions_*.jsonl` 已包含 `readiness_state.version=p1_latent_halt_student_v1` 与 `online_inputs=latent_prefix_and_process_features_only`。主要 loss case：LAMBADA `b -> borneo`，SQuAD `1568– -> 1568–1609` 在 seed66/67 重复，RACE seed68 final/policy 为空但 prediction-stability 为 `A`。结论：P1 locked evidence 支持进入 agent-to-agent v1，但 risk-control 仍要如实声明为 observed-low-risk + partially certified，不是形式化 fully certified safety。
49. 2026-05-27 已完成 P2.3 agent-to-agent latent communication v1 的协议级闭环。新增 `drla/scripts/build_cola_agent_latent_comm_packets.py`，把 locked P1 `halt_decisions_*.jsonl` 与 readiness label 中的 latent refs 合成为 Agent A -> Agent B packet：`latent_memory` 只含 selected prefix block 的 latent `.pt` 引用、block index、shape 与 latent/process features；`readiness_state` 只含 P1 student 多头分数、阈值、margin、halt/fallback 状态，并显式 strip `prediction_stability_block` 等 evaluation-only 字段；`risk_certificate` 只含 fold-level calibration upper-bound certificate；packet schema 明确禁止 decoded text、token ids、scored prediction、official score、gold/target、selected/final/prediction-stability prediction/correctness 等在线字段。全量 locked seeds 66/67/68 official8 test 构建路径 `/data1/luyifei/drla/outputs/cola_agent_latent_comm/p2_agent_latent_comm_v1_locked_seed66_67_68_split20260601_20260527`：`14940` packets、`27399` latent block refs、`8850` unique latent files checked、missing `0`、forbidden key hits `0`；Agent-B v0 receiver hint 统计为 `14043` accept_latent_message、`200` accept_with_uncertified_calibration、`697` accept_final_budget_message。结论：agent-to-agent 的消息协议和可审计数据闭环已落地，但这还不是训练过的 Agent B，也不是 multi-agent accuracy/cost superiority 结论；下一步应在该 packet substrate 上实现 Agent B consume/continue/decode 的真实策略或训练。
50. 如果更改 batch size、block count、decoding 或任务集合，必须重新跑 trace/frontier/train/eval，而不是复用旧结论。在完整链路证据进一步充分后，再考虑 LoRA/adapter/RL-style policy tuning。
```
